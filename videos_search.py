...
#TODO: not yet using radio button.  Price could be better implemented
form_names = ['video_id', 'game_id', 'game_name', 'instance_run_id', 'instance_run_name', 'price']#, 'video_type']
...
<form action="videos.py" method="post">
    <fieldset id="search">
        ...
        <div class="textbox">
            ...
            <input name="video_id" id="video_id" type="text" value={0[0]}>
        </div>
        
         <div class="textbox">
            <label for="game_id">Game ID:</label>
            <input name="game_id" id="game_id" type="text" value={0[1]}>
        </div>

        <div class="textbox">
            <label for="game_name">Game Name:</label>
            <input name="game_name" id="game_name" type="text" value={0[2]}>
        </div>

        <div class="textbox">
            <label for="instance_run_id">InstanceRun ID:</label>
            <input name="instance_run_id" id="instance_run_id" type="text" value={0[3]}>
        </div>

        <div class="textbox">
            <label for="instance_run_name">InstanceRun Name:</label>
            <input name="instance_run_name" id="instance_run_name" type="text" value={0[4]}>
        </div>

         <div class="textbox">
            <label for="price">Price:</label>
            <input name="price" id="price" type="text" value={0[5]}>
        </div>
...
""".format([form_data[name] if form_data[name] else '' for name in form_names])
# .format is used to put previously submitted values into the text boxes, so that they're not lost after a submit

#TODO: connect to the correct database
#db = MySQLdb.connect("info20003db.eng.unimelb.edu.au", "info20003", "cisds", "info20003", 3306)
#cur = db.cursor()
              
#The query will be made up of several elements, starting with lselect
select_elem = 'SELECT'
